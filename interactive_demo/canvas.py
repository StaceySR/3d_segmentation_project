...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.button1 = FocusButton(canvas_frame, text='<', command=self.last_page)
        self.button2 = FocusButton(canvas_frame, text='>', command=self.next_page)
    ...
    def next_page(self):
        if self.page_number < self.imdepth - 1:
            self.page_number = self.page_number + 1
            self.__show_image()
    ...
    def reload_image(self, image, reset_canvas=True):
        ...
        self.__current_image = Image.fromarray(image[self.page_number])

        if reset_canvas:
            self.imdepth, self.imheight, self.imwidth, _ = self.__original_image.shape
    ...
    def __show_image(self):
        ...
        if int(x2 - x1) > 0 and int(y2 - y1) > 0:  # show image if it in the visible area
            ...
            crop_w = min(crop_w, self.__original_image.shape[2] - crop_x)
            crop_h = min(crop_h, self.__original_image.shape[1] - crop_y)

            ori_image = Image.fromarray(self.__original_image[self.page_number])
            __current_image = ori_image.crop((crop_x, crop_y,
    ...
    def _change_canvas_scale(self, relative_scale, x=0, y=0):
        ...
        if new_scale * self.__original_image.shape[2] < self.canvas.winfo_width() and \
           new_scale * self.__original_image.shape[1] < self.canvas.winfo_height():
            return
    ...
    def __size_changed(self):
        new_scale_w = self.canvas.winfo_width() / (self.current_scale * self.__original_image.shape[2])
        new_scale_h = self.canvas.winfo_height() / (self.current_scale * self.__original_image.shape[1])
    ...
    def __left_mouse_button(self, event):
        ...
        if coords is not None:
            self._click_callback(is_positive=True, x=coords[0], y=coords[1], z=self.page_number)
    ...
    def __right_mouse_button_released(self, event):
        ...
        if coords is not None:
            self._click_callback(is_positive=False, x=coords[0], y=coords[1], z=self.page_number)
