...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, batchsize, rows, cols,slices):
        if self.cpu_mode:
            coords = []
            for i in range(batchsize):
                norm_delimeter = self.spatial_scale * self.norm_radius
                coords.append(get_dist_maps(points[i].cpu().float().numpy(), rows, cols, slices,
                                            norm_delimeter))
            ...
            slice_array = torch.arange(start=0, end=slices, step=1, dtype=torch.float32, device=points.device)

            coord_rows, coord_cols, coord_slices = torch.meshgrid(row_array, col_array, slice_array)
            coords = torch.stack((coord_rows, coord_cols, coord_slices), dim=0).unsqueeze(0).repeat(points.size(0), 1, 1, 1, 1)
            ...
            coords[:, 0] = coords[:, 0]+coords[:, 1]+coords[:, 2]
            coords = coords[:, :1]

            coords[invalid_points, :, :, :] = 1e6

            coords = coords.view(-1, num_points, 1, rows, cols,slices)
            coords = coords.min(dim=1)[0]  # -> (bs * num_masks * 2) x 1 x h x w
            coords = coords.view(-1, 2, rows, cols, slices)
