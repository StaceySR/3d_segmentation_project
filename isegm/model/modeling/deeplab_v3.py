...
from .basic_blocks import SeparableConv3d
from .resnet import ResNetBackbone
from isegm.model import ops


class DeepLabV3Plus(nn.Module):
    def __init__(self, backbone='resnet50', norm_layer=nn.BatchNorm3d,
    ...
    def forward(self, x):
        ...
        with ExitStack() as stack:
            ...
            x = self.aspp(c4)
            x = F.interpolate(x, c1.size()[2:], mode='trilinear', align_corners=True)
...
class _DeepLabHead(nn.Module):
    def __init__(self, out_channels, in_channels, mid_channels=256, norm_layer=nn.BatchNorm3d):
        super(_DeepLabHead, self).__init__()

        self.block = nn.Sequential(
            SeparableConv3d(in_channels=in_channels, out_channels=mid_channels, dw_kernel=3,
                            dw_padding=1, activation='relu', norm_layer=norm_layer),
            SeparableConv3d(in_channels=mid_channels, out_channels=mid_channels, dw_kernel=3,
                            dw_padding=1, activation='relu', norm_layer=norm_layer),
            nn.Conv3d(in_channels=mid_channels, out_channels=out_channels, kernel_size=1)
        )
...
class _ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, out_channels=256,
                 project_dropout=0.5, norm_layer=nn.BatchNorm3d):
        ...
        project = [
            nn.Conv3d(in_channels=5*out_channels, out_channels=out_channels,
...
class _AsppPooling(nn.Module):
    ...
    def forward(self, x):
        pool = self.gap(x)
        return F.interpolate(pool, x.size()[2:], mode='trilinear', align_corners=True)
