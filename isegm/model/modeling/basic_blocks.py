...
class ConvHead(nn.Module):
    def __init__(self, out_channels, in_channels=32, num_layers=1,
        ...
        convhead.append(nn.Conv3d(in_channels, out_channels, 1, padding=0))
...
class SepConvHead(nn.Module):
    def __init__(self, num_outputs, in_channels, mid_channels, num_layers=1,
        ...
        for i in range(num_layers):
            sepconvhead.append(
                SeparableConv3d(in_channels=in_channels if i == 0 else mid_channels,
        ...
        sepconvhead.append(
            nn.Conv3d(in_channels=mid_channels, out_channels=num_outputs, kernel_size=1, padding=0)
        )
...
class SeparableConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, dw_kernel, dw_padding, dw_stride=1,
        ...
        _activation = ops.select_activation_function(activation)
        self.body = nn.Sequential(
            nn.Conv3d(in_channels, in_channels, kernel_size=dw_kernel, stride=dw_stride,
                      padding=dw_padding, bias=use_bias, groups=in_channels),
            nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=1, bias=use_bias),
            norm_layer(out_channels) if norm_layer is not None else nn.Identity(),
            _activation()
        )
