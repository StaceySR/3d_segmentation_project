...
class BasicBlockV1b(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None,
                 previous_dilation=1, norm_layer=nn.BatchNorm3d):
...
class ResNetV1b(nn.Module):
    ...
    """
    def __init__(self, block, layers, classes=700, dilated=True, deep_stem=False, stem_width=32,
                 avg_down=False, final_drop=0.0, norm_layer=nn.BatchNorm3d):
        ...
        if not deep_stem:
            ...
            self.conv1 = nn.Sequential(
                nn.Conv3d(3, stem_width, kernel_size=3, stride=2, padding=1, bias=False),
                norm_layer(stem_width),
                nn.ReLU(True),
                nn.Conv3d(stem_width, stem_width, kernel_size=3, stride=1, padding=1, bias=False),
                norm_layer(stem_width),
                nn.ReLU(True),
                nn.Conv3d(stem_width, 2*stem_width, kernel_size=3, stride=1, padding=1, bias=False)
            )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1,
                    avg_down=False, norm_layer=nn.BatchNorm3d):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            if avg_down:
                if dilation == 1:
                    downsample.append(
                        nn.AvgPool3d(kernel_size=stride, stride=stride, ceil_mode=True, count_include_pad=False)
                    )
                else:
                    downsample.append(
                        nn.AvgPool3d(kernel_size=1, stride=1, ceil_mode=True, count_include_pad=False)
                    )
                downsample.extend([
                    nn.Conv3d(self.inplanes, out_channels=planes * block.expansion,
...
def resnet34_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pretrain = torch.load("./weights/r3d34_K_200ep.pth", map_location={'cuda:0': 'cpu'})
