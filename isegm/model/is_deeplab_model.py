...
def get_deeplab_model(backbone='resnet50', deeplab_ch=256, aspp_dropout=0.5,
                      norm_layer=nn.BatchNorm3d, backbone_norm_layer=None,
...
class DistMapsModel(nn.Module):
    def __init__(self, feature_extractor, head, norm_layer=nn.BatchNorm3d, use_rgb_conv=True,
                 cpu_dist_maps=False, norm_radius=260):
        super(DistMapsModel, self).__init__()

        if use_rgb_conv:
            self.rgb_conv = nn.Sequential(
                nn.Conv3d(in_channels=3, out_channels=8, kernel_size=1),
                nn.LeakyReLU(negative_slope=0.2),
                norm_layer(8),
                nn.Conv3d(in_channels=8, out_channels=3, kernel_size=1),
            )
        else:
            self.rgb_conv = None
