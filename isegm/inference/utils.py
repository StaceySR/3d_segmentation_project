...
def compute_noc2_metric(all_ious, max_clicks=20):
    noc_list = [0 for _ in range(max_clicks)]
    count = [0 for _ in range(max_clicks)]
    for i in all_ious:
        for j in range(len(i)):
            noc_list[j] += i[j]
            count[j] += 1
    for i in range(max_clicks):
        noc_list[i] = float(noc_list[i] / count[i])
    return noc_list, count
...
def get_results_table(noc_list, over_max_list, brs_type, dataset_name, mean_spc, elapsed_time,
    ...
    table_row += f'{noc_list[4]:^9.2f}|' if len(noc_list) > 4 else f'{"?":^9}|'
    table_row += f'{over_max_list[0]:^9}|'
    ...
    table_row += f'{over_max_list[4]:^9}|' if len(noc_list) > 4 else f'{"?":^9}|'
