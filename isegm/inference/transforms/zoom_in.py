...
class ZoomIn(BaseTransform):
    ...
    def _transform_clicks(self, clicks_list):
        ...
        rmin, rmax, cmin, cmax = self._object_roi
        crop_height, crop_width, depth = self._roi_image.shape[2:]

        transformed_clicks = []
        for click in clicks_list:
            new_r = crop_height * (click.coords[0] - rmin) / (rmax - rmin + 1)
            new_c = crop_width * (click.coords[1] - cmin) / (cmax - cmin + 1)
            transformed_clicks.append(Click(is_positive=click.is_positive, coords=(new_r, new_c, click.coords[2])))
...
def get_roi_image_nd(image_nd, object_roi, target_size):
    ...
    height = rmax - rmin + 1
    width = cmax - cmin + 1
    depth = image_nd.shape[4]

    if isinstance(target_size, tuple):
        new_height, new_width = target_size
        new_depth = image_nd.shape[4] * new_height / height
    else:
        scale = target_size / max(height, width)
        new_height = int(round(height * scale))
        new_width = int(round(width * scale))
        new_depth = int(round(depth * scale))

    with torch.no_grad():
        roi_image_nd = image_nd[:, :, rmin:rmax + 1, cmin:cmax + 1, 0:depth]
        # roi_image_nd = torch.nn.functional.interpolate(roi_image_nd, size=(new_height, new_width, new_depth),
        #                                                mode='trilinear', align_corners=True)
