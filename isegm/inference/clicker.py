...
class Clicker(object):
    ...
    def _get_click(self, pred_mask, padding=True):
        ...
        if padding:
            fn_mask = np.pad(fn_mask, ((1, 1), (1, 1), (1, 1)), 'constant')
            fp_mask = np.pad(fp_mask, ((1, 1), (1, 1), (1, 1)), 'constant')

        fn_mask_dt = distance_transform_edt(fn_mask)
        fp_mask_dt = distance_transform_edt(fp_mask)

        if padding:
            fn_mask_dt = fn_mask_dt[1:-1, 1:-1, 1:-1]
            fp_mask_dt = fp_mask_dt[1:-1, 1:-1, 1:-1]
        ...
        if is_positive:
            coords_z, coords_y, coords_x = np.where(fn_mask_dt == fn_max_dist)  # coords is [y, x]
        else:
            coords_z, coords_y, coords_x = np.where(fp_mask_dt == fp_max_dist)  # coords is [y, x]

        return Click(is_positive=is_positive, coords=(coords_z[0], coords_y[0], coords_x[0]))
    ...
    def add_click(self, click):
        ...
        if self.gt_mask is not None:
            self.not_clicked_map[coords[0], coords[1], coords[2]] = False
    ...
    def _remove_last_click(self):
        ...
        if self.gt_mask is not None:
            self.not_clicked_map[coords[0], coords[1], coords[2]] = True
