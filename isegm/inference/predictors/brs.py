        ...
        for list_indx, clicks_list in enumerate(clicks_lists):
            for click in clicks_list:
                ...
                if click.is_positive:
                    pos_clicks_map[list_indx, 0, y1:y2, x1:x2, z] = True
                else:
                    neg_clicks_map[list_indx, 0, y1:y2, x1:x2, z] = True
...
class FeatureBRSPredictor(BRSBasePredictor):
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        def get_prediction_logits(scale, bias):
            scale = scale.view(bs, -1, 1, 1, 1)
            bias = bias.view(bs, -1, 1, 1, 1)
            if self.with_flip:
                scale = scale.repeat(2, 1, 1, 1, 1)
                bias = bias.repeat(2, 1, 1, 1, 1)
            ...
            if self.insertion_mode == 'after_c4':
                ...
                x = F.interpolate(x, mode='trilinear', size=self._c1_features.size()[2:],
                                  align_corners=True)
            ...
            pred_logits = F.interpolate(pred_logits, size=image_nd.size()[2:], mode='trilinear',
                                        align_corners=True)
            return pred_logits
        ...
        with torch.no_grad():
            ...
            if self.insertion_mode == 'after_c4' or self.insertion_mode == 'after_aspp':
                ...
                if self.insertion_mode == 'after_aspp':
                    ...
                    x = F.interpolate(x, size=c1.size()[2:], mode='trilinear', align_corners=True)
